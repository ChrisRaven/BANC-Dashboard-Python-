...
def create_synaptic_partners_section(root):
  ...
  def filter_dust_handler():
    ...
    frags_threshold = fragments_threshold.get().strip()
    source_group = common_source_selection.get_selected()
    ...
    source_text = input_ids.get('1.0', 'end').strip()
    source_ids = clean_input(source_text)
    filter_by_no_of_fragments(source_group, int(min_size), int(frags_threshold), source_ids, filter_dust_callback)
  ...
  def filter_dust_callback(result):
    if isinstance(result, str) and result.startswith('MSG:'):
      ...
      match msg_type:
        case 'IN_PROGRESS':
          insert(filtered_partners_smaller, msg_content)
        case 'COMPLETE':
          hide_loading_indicator()
          insert(filtered_partners_smaller, '\n'.join(map(str, result["smaller"])))
          insert(filtered_partners_larger, '\n'.join(map(str, result["larger"])))
        case 'ERROR':
          hide_loading_indicator()
          insert(filtered_partners_smaller, msg_content)
    else:
  ...
  fragments_threshold_wrapper = widgets.column(parent=filter_wrapper)
  fragments_threshold = widgets.labeledEntry(parent=fragments_threshold_wrapper, label="Frags", default_value='100')

  

  filtered_results_wrapper = widgets.column_wrapper(parent=border3)
  smaller_column = widgets.column(parent=filtered_results_wrapper)
  filtered_partners_smaller = widgets.countTextbox(parent=smaller_column, label='Smaller')
  larger_column = widgets.column(parent=filtered_results_wrapper)
  filtered_partners_larger = widgets.countTextbox(parent=larger_column, label='Larger')
